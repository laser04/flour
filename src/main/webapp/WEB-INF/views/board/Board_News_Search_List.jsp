...
<body>
   <div class="mx-auto" style="width: 1350px;">
      ...
      <c:if test="${startPage > pageblock}">
         <div>
         <a href="<%=request.getContextPath() %>/boardnewslist/search?pageNum=${startPage-pageblock}&searchKeyword=${searchKeyword}&searchCondition=${searchCondition}">&lt;back
         </a></div><br><br>
      </c:if>
      
      <c:forEach var="i" begin="${startPage}" end="${endPage}">
           <div>
           <a href="<%=request.getContextPath() %>/boardnewslist/search?pageNum=${i}&searchKeyword=${searchKeyword}&searchCondition=${searchCondition}">${i}</a>
           </div>
      </c:forEach>
      
      <c:if test="${endPage < pageCount }">
      <br><br><div><a 
         href="<%=request.getContextPath() %>/boardnewslist/search?pageNum=${startPage+pageblock}&searchKeyword=${searchKeyword}&searchCondition=${searchCondition}">next&gt;
      </a></div>
      </c:if>
