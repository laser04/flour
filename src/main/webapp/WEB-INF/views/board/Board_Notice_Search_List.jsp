   ...
   <div class="mx-auto" style="width: 1350px;">
      ...
      <div class="mx-auto bg-white rounded mt-5 p-5" style="box-shadow: 0px 2px 10px rgba(0, 0, 0, 0.1);">
         <div class="d-flex justify-content-between mx-auto"
            style="width: 90%;">
            <div class="text-info fs-2 fw-bolder">공지사항</div>
            
            <form action="<%=request.getContextPath() %>/boardnoticelist/search" 
            ...
            <div class="d-flex col-8 align-bottom justify-content-end"
               ...
               <select class="form-select w-25" name="searchCondition">
                  <option value="BOARDNOTICETITLE" selected>제목</option>
         ...
         <div class="mx-auto mt-4" style="width: 90%;">
            <table id="table-board"
               ...
               <c:forEach items="${boardNoticeList}" var="boardnotice">
                  <tr> 
                     <td class="text-start"><a href="<%=request.getContextPath() %>/boardnoticeget?BOARDNOTICEID=${boardnotice.BOARDNOTICEID}">${boardnotice.BOARDNOTICETITLE}</a></td>
                     <td>${boardnotice.USERNAME}</td>
                     <td>${boardnotice.BOARDNOTICEREGDATE}</td>
                     <td>${boardnotice.BOARDNOTICECOUNT}</td>
                  </tr>
               </c:forEach>
      ...
      <div id="page">
         <c:if test="${startPage > pageblock}">
            <div>
            <a href="<%=request.getContextPath() %>/boardnoticelist/search?pageNum=${startPage-pageblock}&searchKeyword=${searchKeyword}&searchCondition=${searchCondition}">&lt;back
            </a></div><br><br>
         </c:if>
         
         <c:forEach var="i" begin="${startPage}" end="${endPage}">
              <div>
              <a href="<%=request.getContextPath() %>/boardnoticelist/search?pageNum=${i}&searchKeyword=${searchKeyword}&searchCondition=${searchCondition}">${i}</a>
              </div>
         </c:forEach>
         
         <c:if test="${endPage < pageCount }">
         <br><br><div><a 
            href="<%=request.getContextPath() %>/boardnoticelist/search?pageNum=${startPage+pageblock}&searchKeyword=${searchKeyword}&searchCondition=${searchCondition}">next&gt;
         </a></div>
         </c:if>
         </div>
      </div>
