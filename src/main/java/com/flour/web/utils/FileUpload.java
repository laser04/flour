...
public class FileUpload {
	...
	public	Board fileupload(String contextPath,MultipartFile uploadfile,Board dto) throws Exception {
		//파일 업로드	이부분을  따로 뺴서 클래스 만들기 
		
				if(!uploadfile.isEmpty()) {
					
					//getRealPath() 메소드는 웹 애플리케이션의 루트 경로를 반환합니다
					Board filedto=new Board(contextPath
					...
					//데이터베이스에 저장
					dto.setBoardFileName(filedto.getFileName());
					dto.setBoardFilePath(filedto.getUuid()+"_"
							+filedto.getFileName());
