...
<body>
	<div class="mx-auto" style="width: 1350px;">
		...
		<div class="mx-auto bg-white rounded mt-5 p-5" style="box-shadow: 0px 2px 10px rgba(0, 0, 0, 0.1);">
			<div class="d-flex justify-content-between mx-auto"
				...
				<form action="<%=request.getContextPath() %>/boardlist/search" 
				name="frm1">
			...
			<div class="mx-auto mt-4" style="width: 90%;">
				...
				<c:forEach items="${boardList}" var="board">
					<tr>
						...
						<td class="text-start"><a href="<%=request.getContextPath() %>/boardget?BOARDID=${board.BOARDID}">${board.BOARDTITLE}</a></td>
		...
		<c:if test="${startPage > pageblock}">
			<div>
			<a href="<%=request.getContextPath() %>/boardlist/search?
			pageNum=${startPage-pageblock}&searchKeyword=${searchKeyword}
			&searchCondition=${searchCondition}">&lt;back
			</a></div><br><br>
		</c:if>
		
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
  			<div>
  			<a href="<%=request.getContextPath() %>/boardlist/search?pageNum=${i}&searchKeyword=${searchKeyword}
			&searchCondition=${searchCondition}">${i}</a>
  			</div>
		</c:forEach>
		
		<c:if test="${endPage < pageCount }">
		<br><br><div><a 
			href="<%=request.getContextPath() %>/boardlist/search?
			pageNum=${startPage+pageblock}&searchKeyword=${searchKeyword}
			&searchCondition=${searchCondition}">next&gt;
		</a></div>
		</c:if>
	...
	<script type="text/javascript">
		...
		function boardinsert() {
	        window.location.href = "<%=request.getContextPath()%>/move_boardinsert";
	    }
	</script>
