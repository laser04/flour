...
<body>
	<div class="mx-auto" style="width: 1350px;">
		...
		<c:if test="${startPage > pageblock}">
			<div>
			<a href="<%=request.getContextPath() %>/boardlist?pageNum=${startPage-pageblock}">&lt;back
			</a></div><br><br>
		</c:if>
		
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
  			<div>
  			<a href="<%=request.getContextPath() %>/boardlist?pageNum=${i }">${i }</a>
  			</div>
		</c:forEach>
		
		<c:if test="${endPage < pageCount }">
		<br><br><div><a 
			href="<%=request.getContextPath() %>/boardlist?pageNum=${startPage+pageblock}">next&gt;
		</a></div>
		</c:if>
